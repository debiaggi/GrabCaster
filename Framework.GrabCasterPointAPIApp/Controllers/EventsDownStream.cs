    ...
    public class EventsDownStream
    {
        ...
        public void Run(MessageIngestor.SetEventActionEventEmbedded setEventActionEventEmbedded)
        {
            try
            {
                ...
                LogEngine.WriteLog($"Start GrabCaster DownStream - Point Id {ConfigurationLibrary.PointId()} - Point name {ConfigurationLibrary.PointName()} - Channel Id {ConfigurationLibrary.ChannelId()} - Channel name {ConfigurationLibrary.ChannelName()} ");
                ...
                LogEngine.WriteLog($"Start DirectRegisterEventReceiving. - Initializing Group Name {eventHubConsumerGroup}");
                ...
                LogEngine.WriteLog("After DirectRegisterEventReceiving Downstream running.");
            }
            catch (Exception ex)
            {
                LogEngine.WriteLog($"Error in {MethodBase.GetCurrentMethod().Name} - Hint: Check if the firewall outbound port 5671 is opened. - Error {ex.Message}");
            }
        }
            ...
            string consumerGroup)
        {
            try
            {
                ...
                LogEngine.WriteLog($"Direct Receiver created. Partition {partitionId}");
                while (true)
                {
